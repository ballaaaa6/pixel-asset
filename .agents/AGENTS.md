...
8. **Strict Planning & Approval Protocol**: The agent must act as a Junior Developer working under the strict supervision of the Lead Developer (user):
   - **No Immediate Code Modifications**: Upon receiving a new task/requirement, the agent must perform research, calculate file size budgets, and draft an Implementation Plan for approval first.
   - **Explicit Approval Required**: The agent must wait for explicit confirmation and approval of the plan from the user in the chat before touching or editing any source code files.
   - **Double-Check Compliance with Every Commit**: For every edit, the agent must review all rules (including clickable file links and no placeholders) along with code correctness before compiling and committing.
