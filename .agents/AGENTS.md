...
6. **Modular Architecture & File Size Limits**: Keep UI components, hooks, and serverless backend API functions focused and lightweight (< 400 lines). Avoid creating massive monolithic files. Place charts, shared components, and dashboard sub-components in their respective subdirectories (`src/components/charts/`, `src/components/common/`, `src/components/dashboard/`). Place interaction and state logic in `src/hooks/`. Extract formatting helpers to `src/utils/formatters.js` and shared business logic to `src/utils/assetHelpers.js`. For backend Pages Functions (`functions/api/`), extract helper logic to files prefixed with an underscore (e.g. `_pricesHelpers.js`) to ensure they are ignored by the Cloudflare Pages router. The pre-build script `scripts/check-file-size.js` enforces this limit on all JS/JSX source files. Refer to [ARCHITECTURAL_GUIDE.md](file:///d:/antigravity/us%20stock%20tracker/ARCHITECTURAL_GUIDE.md) for structural mapping.
7. **Proactive Size Estimation**: Before modifying any file (frontend or backend), the agent must check the current line count and calculate the projected post-edit line count (`Current + Inserted - Deleted`). If the projected line count exceeds 400 lines or gets close to the limit (380-400 lines), the agent must plan to split functions or components into sub-modules **before** starting to write the actual code. This ensures proactive prevention rather than reactive fixing after a build failure.
